...
def fit_lstm(train, n_lag, n_seq, n_batch, nb_epoch, n_neurons):
    ...
    for i in range(nb_epoch):
        if i % 25 == 0:
            print(str(i) + '/' + str(nb_epoch) + ' epoch')
...
def save_model_to_file(model, filename_wo_extension = 'model'):
    ...
    with open(filename_wo_extension + '.json', 'w') as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save_weights(filename_wo_extension + '.h5')
...
if input('save model? (y/n)') == 'y':
    save_model_to_file(model, 'model'+ datetime.now().strftime('%Y-%m-%d-%H_%M_%S'))
